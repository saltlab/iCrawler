...
- (BOOL)isEdgeUniqueWithFrom:(NSUInteger)from to:(NSUInteger)to command:(NSString*)monkeyCommand;
...
@synthesize isDigraph, nodeEdges;
...
- (void)addEdgeFrom:(NSUInteger)from to:(NSUInteger)to command:(NSString*)aMonkeyCommand{
    GraphEdge *edge = [[GraphEdge alloc] initWithFrom:from to:to command:aMonkeyCommand];
    [self addEdge:edge];
    [edge release];
}
...
//    }
    if ([self isEdgeUniqueWithFrom:edge.from to:edge.to command:edge.monkeyCommand]) {
        [[nodeEdges objectAtIndex:edge.from] addObject:edge];
    }
...
- (BOOL)isEdgeUniqueWithFrom:(NSUInteger)from to:(NSUInteger)to command:(NSString*)monkeyCommand {
    ...
    NSMutableArray *edges = [nodeEdges objectAtIndex:from];
    for (GraphEdge *edge in edges) {
        if (edge.to == to && [edge.monkeyCommand isEqualToString:monkeyCommand]) {
            return NO;
        }
    }
    return TRUE;
