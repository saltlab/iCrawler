...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString*)identifier {
  if (self = [super initWithStyle:UITableViewCellStyleValue2 reuseIdentifier:identifier]) {
    ...
    self.textLabel.highlightedTextColor = TTSTYLEVAR(highlightedTextColor);
    self.textLabel.textAlignment = UITextAlignmentLeft;
	self.textLabel.lineBreakMode = UILineBreakModeWordWrap; //UILineBreakModeTailTruncation;
    self.textLabel.adjustsFontSizeToFitWidth = YES;
...
//		_imageView2.frame = CGRectMake(kHPadding, floor(self.height/2 - iconHeight/2),
  //                                 iconWidth, iconHeight);
      ...
      CGFloat innerHeight = self.contentView.height - kVPadding*2;
      self.textLabel.frame = CGRectMake(kHPadding + iconWidth + kKeySpacing - 15 , kVPadding,
...
+ (CGFloat)tableView:(UITableView*)tableView rowHeightForObject:(id)object {
	...
	CGFloat padding = [tableView tableCellMargin]*2 + item.padding.left + item.padding.right + 30;
	if (item.URL) {
		padding += kDisclosureIndicatorWidth;
	}
	
	item.text.width = tableView.width - padding ;
	//NSLog(@"text = %@ width = %f item row = %f padding left %f padding right %f paddg top %f padding bottom %f ", item.text, item.text.width, item.text.height + item.padding.top + item.padding.bottom, item.padding.left, item.padding.right,  item.padding.top , item.padding.bottom);
	return item.text.height + item.padding.top + item.padding.bottom  + 20;
...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString*)identifier {
	if (self = [super initWithStyle:UITableViewCellStyleValue2 reuseIdentifier:identifier]) {
		...
		self.textLabel.font = TTSTYLEVAR(font);
		self.textLabel.backgroundColor = TTSTYLEVAR(backgroundColor);//
		self.textLabel.textColor = TTSTYLEVAR(textColor);
		self.textLabel.highlightedTextColor = TTSTYLEVAR(highlightedTextColor);
		self.textLabel.textAlignment = UITextAlignmentLeft;
		self.textLabel.lineBreakMode = UILineBreakModeTailTruncation;
...
- (void)layoutSubviews {
	...
	if ([self.detailedTextLabel text] != nil) {
		CGFloat textHeight = [[self.detailedTextLabel font] lineHeight] * (kMessageTextLineCount + (self.captionLabel.text.length ? 0 : 1));
		
		self.detailedTextLabel.frame = CGRectMake(left, top, width, self.contentView.bounds.size.height - top);
...
- (UILabel*)titleLabel {
	if (!_titleLabel) {
		_titleLabel = [[UILabel alloc] init];
		//Changed by GH
		_titleLabel.textColor = TTSTYLEVAR(styledMessageTitleColor);//[UIColor whiteColor];
...
- (UILabel*)timestampLabel {
	if (!_timestampLabel) {
		...
		_timestampLabel.highlightedTextColor = [UIColor whiteColor];
		_timestampLabel.contentMode = UIViewContentModeLeft;
		_timestampLabel.backgroundColor = [UIColor blueColor];
		[self.contentView addSubview:_timestampLabel];
	}
	return _timestampLabel;
}
...
- (TTStyledTextLabel*)detailedTextLabel {
	if(!_detailedTextLabel) {
		_detailedTextLabel = [[TTStyledTextLabel alloc] initWithFrame:CGRectMake(self.contentView.frame.origin.x, self.contentView.frame.origin.y, self.contentView.frame.size.width,self.contentView.frame.size.height + 100)];
		_detailedTextLabel.font = [UIFont systemFontOfSize:12];
		[self.contentView addSubview:_detailedTextLabel];
//		NSLog(@"%f %f %f %f", _detailedTextLabel.frame.origin.x,_detailedTextLabel.frame.origin.y, _detailedTextLabel.frame.size.width, _detailedTextLabel.frame.size.height);
